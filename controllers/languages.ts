  ...
  try {
    ...
    if (code) {
      if (
        ...
        return res
          .status(400)
          .json({ error: "Code must be a non-empty string without spaces" });
      }
    }
    ...
    if (name) {
      if (
        ...
        return res
          .status(400)
          .json({ error: "Name must be a non-empty string without spaces" });
      }
    }
...
export const addLanguage = async (req: Request, res: Response) => {
  try {
    ...
    if (name.length > 255 || code.length > 16) {
      return res
        .status(400)
        .json({ error: "Name or code exceeds maximum length" });
    }
    if (
      typeof name !== "string" ||
      name.trim().includes(" ") ||
      ...
      return res
        .status(400)
        .json({
          error: "Name and code must be non-empty strings without spaces",
        });
    }
